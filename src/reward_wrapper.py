...
class PickKey(Wrapper):
    ...
    def step(self, action):
        ...
        if action == self.unwrapped.actions.pickup and self.unwrapped.carrying and isinstance(self.unwrapped.carrying, Key):
        ...
        if action == self.unwrapped.actions.toggle:
            ...
            if front_cell and isinstance(front_cell, Door):
                if front_cell.is_locked and self.unwrapped.carrying and isinstance(self.unwrapped.carrying, Key):
        ...
        if terminated and self.unwrapped.agent_pos == (self.unwrapped.width - 2, self.unwrapped.height - 2):
            reward = 10  # Assign a reward for reaching the goal
        else:
